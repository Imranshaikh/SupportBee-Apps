module Smsway
  module EventHandler
    def ticket_created
      ...
      sent_sms("New ticket from #{ticket.requester.name || ticket.requester.email} - #{ticket.subject}")
    end

    def customer_reply_created
      return true unless settings.notify_customer_reply_created.to_s == '1'
      reply = payload.reply
      sent_sms("New customer reply from #{reply.replier.name || reply.replier.email} in #{payload.ticket.subject}")
    end

    def agent_reply_created
      return true unless settings.notify_agent_reply_created.to_s == '1'
      reply = payload.reply
      sent_sms("New agent reply from #{reply.replier.name || reply.replier.email} in #{payload.ticket.subject}")
    end

    def comment_created
      return true unless settings.notify_comment_created.to_s == '1'
      comment = payload.comment
      sent_sms("New comment from #{comment.commenter.name || comment.commenter.email} in #{payload.ticket.subject}")
    end
  end
end
